...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sT->GetEnableTransmogInfo())
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t如何幻化装备外观?", EQUIPMENT_SLOT_END + 9, 0);
...
#endif
        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t取消所有幻化", EQUIPMENT_SLOT_END + 2, 0, "取消所有已穿戴的幻化", 0, false);
        ...
        switch (sender)
        {
            ...
            case EQUIPMENT_SLOT_END + 4: // Presets menu
            {
                ...
                if (sT->GetEnableSetInfo())
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t如何使用?", EQUIPMENT_SLOT_END + 10, 0);
            ...
            case EQUIPMENT_SLOT_END + 6: // view preset
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|t使用这套方案", EQUIPMENT_SLOT_END + 5, action, "使用装备用于外观修改，会让修改后的装备与你绑定且无法退还.\n是否继续?\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|t删除设置", EQUIPMENT_SLOT_END + 7, action, "确定要删除" + sT->presetByName[player->GetGUID()][action] + "吗?", 0, false);
    ...
    void ShowTransmogItems(Player* player, Creature* creature, uint8 slot, uint16 gossipPageNumber) // Only checks bags while can use an item from anywhere in inventory
    {
        ...
        if (oldItem)
        {
            ...
            if (sT->GetUseCollectionSystem())
            {
                ...
                if (sT->collectionCache.find(accountId) != sT->collectionCache.end())
                {
                    ...
                    if (sT->GetAllowHiddenTransmog())
                    {
                        ...
                        else
                        {
                            ...
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "隐藏部位", slot, UINT_MAX, "隐藏这个部位.\n是否继续?\n\n" + lineEnd, 0, false);
                        }
                    }
                    ...
                    for (uint32 i = startValue; i <= endValue; i++)
                    {
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetEntry(), "使用装备用于外观修改，会让修改后的装备与你绑定且无法退还.\n是否继续?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                    }
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t取消幻化", EQUIPMENT_SLOT_END + 3, slot, "取消该位置的幻化", 0, false);
            ...
            else
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "使用装备用于外观修改，会让修改后的装备与你绑定且无法退还.\n是否继续?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                }
...
private:
    ...
    void AddToDatabase(Player* player, ItemTemplate const* itemTemplate)
    {
        ...
        {
            if (showChatMessage)
                ChatHandler(player->GetSession()).PSendSysMessage( R"(|c%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r 已添加到你的幻化外观收藏中.)", itemQuality.c_str(), itemId, itemName.c_str());
